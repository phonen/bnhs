...
//             $post_data['voice_url'] = trim($post_data['voice_url'],'&quot;');
            ...
            if(!$samplepy){
               //删除没有说话后的文件
                   ...
                   file_put_contents("/www/wwwroot/1111zn.com/application/Api/Controller/删除pcm记录.txt",serialize($pcm_file));
                   file_put_contents("/www/wwwroot/1111zn.com/application/Api/Controller/删除MP3记录.txt",serialize($mp3_file));
                   $cmd = "rm -f  $pcm_file" ; 
                   $res = shell_exec($cmd);
                   $cmd = "rm -f  $mp3_file" ; 
                   $res = shell_exec($cmd);
