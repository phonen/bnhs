...
use Common\Lib\Baidu\Btt;//新增的百度识别类
 ...
 /*   	$adv = AdvController::instance()->getAdv('hbsquare');exit(json_encode($adv));exit();
    ...
    public function saveEnveReceive(){
    	$post_data = I('post.');  
     	$pid = I('post.pid/d');
         	$pid = I('post.pid/d');
      $home ="/www/wwwroot/1111zn.com";
      	$zm_path =substr($post_data['voice_url'],0,30);
        $zm_filename = substr($post_data['voice_url'],30);
     	
     
		$cmd ="/www/wwwroot/1111zn.com/silk-v3/converter.sh {$home}{$zm_path}{$zm_filename} wav"; 
        $res = shell_exec($cmd);
     	
        $enve_model = M("Enve");
        $expire = 86400;
        $info = $enve_model->where("id='%d' and del=0 and pay_status='%s'",array($pid, 'ok'))->find();
        if(!$info){ $this->ajaxReturn(['code'=>40500, 'msg'=>'不是有效红包!']); }
        if($info['be_overdue']==1){       $this->ajaxReturn(['code'=>40500, 'msg'=>'红包已失效!']); } 
      
        $enve_receive_model = D("EnveReceive"); 
        $count = $enve_receive_model->where("pid='%d' and user_id='%s'",array($pid, $this->user_id))->count();
        if($count >  0){   $this->ajaxReturn(['code'=>40500, 'msg'=>'您已领此取过红包']); }

       
        if ($info['enve_type'] != 1) { 

      		$filename = trim(substr($post_data['voice_url'],30),'.silk');
      		$zm_wav_file ="{$home}{$zm_path}{$filename}.wav";
          
          
          
            //file_put_contents("/www/wwwroot/1111zn.com/application/Common/Lib/Baidu/data.txt",serialize($zm_wav_file));
          	$baidu_obj = new Btt();//从类获取
    		$baidu_token = $baidu_obj->get_token();
          $baidu_res = $baidu_obj->WavTotext($baidu_token,$zm_wav_file);
          	 $baiduress=json_decode($baidu_res,true);
          	if ($baidu_res){
         		$str = str_replace("，", '', $baiduress["result"][0]);
          	   		$zhwjdz = "/www/wwwroot/1111zn.com/data/upload/";
            		file_put_contents($zhwjdz, $baidu_res);
             	   // $this->ajaxReturn(['code'=>2000, 'msg'=>"$str"]);         
         		 }else{            	
            		$this->ajaxReturn(['code'=>2000, 'msg'=>"请将普通话"]);   
         		 } 
           ...
           // $samplepy = $py->getPY($sampleText);
          	$samplepy = $py->getPY($str);

            if(!$samplepy){
                $this->ajaxReturn(['code'=>40000, 'msg'=>'您没有说话哟']);
            };
            $compareStr = '';
            if ($info['enve_type'] == 0) {
