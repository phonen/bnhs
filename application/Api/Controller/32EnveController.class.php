...
class EnveController extends InterceptController {
 ...
 /*   	$adv = AdvController::instance()->getAdv('hbsquare');exit(json_encode($adv));exit();
    ...
    public function saveEnveReceive(){
		...
		//$cmd ="/www/wwwroot/1111zn.com/silk-v3/converter.sh {$home}{$zm_path}{$zm_filename} mp3"; 
       // $res = shell_exec($cmd);
        ...
        if ($info['enve_type'] != 1) {
          ...
          $post_data['voice_wav'] = trim($zm_wav_file,'/');
//           
             //$post_data['voice_url'] = $zm_wav_file;
             file_put_contents("/www/wwwroot/1111zn.com/application/Common/Lib/Baidu/data44.txt",serialize($post_data['voice_url']));
